      ...
      <div className="container w-11/12 mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-10">
          {researchAreas.areas.map((area, index) => (
            <Link 
              key={area.id}
              to={area.id === "Tools & Software" ? "/software" : `/research/${area.id}`}
              className="block"
            >
              <motion.div 
                ...
                whileHover={{ y: -5 }} 
                className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 flex flex-col h-full cursor-pointer"
              >
